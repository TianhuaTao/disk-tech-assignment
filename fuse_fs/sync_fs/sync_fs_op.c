 ...
 * */
int sync_mkdir(const char *, mode_t);

/** Remove a file */
int sync_unlink(const char *name){
    return unlink(name);
}
 ...
 */
int sync_chown(const char *, uid_t, gid_t, struct fuse_file_info *fi);
 ...
 * expected to reset the setuid and setgid bits.
 */
int sync_truncate(const char *, off_t, struct fuse_file_info *fi);
 ...
 *
 */
int sync_open(const char *file, struct fuse_file_info *info){
    int fd = open(file, info->flags);
    info -> fh=fd;
    return fd;
}
 ...
 */
int sync_create(const char *, mode_t, struct fuse_file_info *);
