...
RetCode EngineRace::read_index_file(int bucket_id, int &fd)
{
  ...
  while ((rd = read(fd, (char *)&sz, 4)) > 0)
  {
    // log(std::to_string(cnt)+": "+std::to_string(rd)+" read");
    if (rd != 4)
    {
    ...
    if (rd != sz)
    {
      // log("rd: "+std::to_string(rd)+" sz: "+std::to_string(sz));
  ...
  for (size_t i = 0; i < thread_cnt; i++)
  {
    ...
    else
    {
      ...
      if (fd_index < 0)
      {
        ...
        // log(std::to_string(lseek(fd_index_tmp, 0, SEEK_CUR)));
        RetCode ret = engine_race->read_index_file(i, fd_index_tmp);
        // log("Read index.tmp done");
        if (ret != kSucc)
          return ret;
    ...
    // signal(9,[](int){fsync(EngineRace::fd_index_tmp);});
...
// 2. Close engine
// write index hash map to file, replace the temporary index file
...
EngineRace::~EngineRace()
{
  ...
  for (size_t i = 0; i < thread_cnt; i++)
  {
    ...
    fsync(fd_index_tmp[i]);
    close(fd_index_tmp[i]);
    remove((dir_name + "/index_" + std::to_string(i) + ".tmp").c_str());
  }
...
RetCode EngineRace::Write(const PolarString &key, const PolarString &value)
{
  ...
  FileAppend(fd_data[bucket_id], value.ToString()); // write data
...
RetCode EngineRace::Read(const PolarString &key, std::string *value)
{
  ...
  // log("offset: "+ std::to_string(loc.offset)+" len: "+std::to_string(loc.len));
